...
function buildKeyOptions(): Array<{
  name: string;
  description: string;
  value: string;
}> {
...
function buildMainMenu(): any {
  ...
  if (mainMenuIndex >= options.length)
    mainMenuIndex = Math.max(0, options.length - 1);
...
function buildKeySelector(): any {
  ...
  if (keySelectorIndex >= options.length)
    keySelectorIndex = Math.max(0, options.length - 1);
  ...
  selector.on(
    "itemSelected" as any,
    (index: number, option: { value: string }) => {
...
function buildKeyActions(): any {
  ...
  if (keyActionsIndex >= options.length)
    keyActionsIndex = Math.max(0, options.length - 1);
...
function buildRenameInput(): any {
  const theme = t();
  if (!renameTargetId)
    return Text({ content: "Error: no key selected", fg: theme.error });
  ...
  switch (currentScreen) {
    ...
    case "add-name":
      content = Box(
        { flexDirection: "column", gap: 1 },
        Text({
          content: "Enter a friendly name for this key:",
          fg: theme.text,
        }),
        buildAddNameInput(),
      );
