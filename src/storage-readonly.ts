...
export function loadStoreReadonly(configDir?: string): KeyStore | null {
  ...
  try {
    ...
    return {
      ...
      fallbackChain: Array.isArray(data.fallbackChain) ? data.fallbackChain : [],
      maxRateLimitFailures: typeof data.maxRateLimitFailures === "number" ? data.maxRateLimitFailures : 3,
    } as KeyStore;
  } catch {
    return null;
  }
...
export function formatKeyStatus(keys: ApiKeyEntry[], now: number): string {
  ...
  for (const k of keys) {
    ...
    const blStr = blacklisted.length > 0 ? ` BLACKLISTED: ${blacklisted.join(", ")}` : "";
...
export function getBlacklistedModels(
  keys: ApiKeyEntry[],
  now: number,
): Array<{ keyId: string; keyName: string; modelId: string; remainingSecs: number; nextDurationMs: number }> {
  const results: Array<{ keyId: string; keyName: string; modelId: string; remainingSecs: number; nextDurationMs: number }> = [];
  for (const k of keys) {
