...
export function resolveStorePath(config?: KeyStoreConfig): string {
  return config?.storePath ?? process.env.NIM_ROTATOR_STORE_PATH ?? DEFAULT_STORE_PATH
}
...
export function toggleKey(store: KeyStore, id: string, enabled?: boolean): void {
  const entry = store.keys.find((k) => k.id === id)
  if (entry) entry.enabled = enabled ?? !entry.enabled
}

export function getMaxFailures(config?: KeyStoreConfig): number {
  return config?.maxFailuresBeforeDisable ?? (Number(process.env.NIM_ROTATOR_MAX_FAILURES) || DEFAULT_MAX_FAILURES)
}

export function getActiveKeys(store: KeyStore, config?: KeyStoreConfig): ApiKeyEntry[] {
  const maxFailures = getMaxFailures(config)
  return store.keys.filter((k) => k.enabled && k.failureCount < maxFailures)
}

export function getNextKey(store: KeyStore, config?: KeyStoreConfig): { key: ApiKeyEntry; index: number } | null {
