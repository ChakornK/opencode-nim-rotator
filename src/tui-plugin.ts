...
function showStatusDialog(api: TuiPluginApi) {
	...
	if (!store) {
		api.ui.toast({
			...
			message:
				"No key store found. Run `bun opencode-nim-rotator` in a terminal to add keys.",
			duration: 5000,
		});
		return;
	}
	...
	api.ui.dialog.replace(() => {
		// biome-ignore lint/suspicious/noExplicitAny: DialogSelect is a runtime method from @opentui/core not in TuiPluginApi types
		return (api.ui as any).DialogSelect({
			...
			// biome-ignore lint/suspicious/noExplicitAny: return type matches what dialog.replace expects
		}) as any;
	});
}
...
function showResetAllConfirmDialog(api: TuiPluginApi) {
	api.ui.dialog.replace(() => {
		// biome-ignore lint/suspicious/noExplicitAny: DialogConfirm is a runtime method from @opentui/core not in TuiPluginApi types
		return (api.ui as any).DialogConfirm({
			title: "Reset All Rate Limits?",
			message:
				"This will reset all rate limit counts and clear all model blacklists for every key.",
			onConfirm: () => {
				const configDir = getConfigDir(api);
				const store = loadFullStore(configDir);
				if (store) {
...
function showFallbackChainDialog(api: TuiPluginApi) {
	...
	if (chain.length === 0) {
		...
		api.ui.toast({
			...
			title: "NIM Key Rotator",
			message:
				"No fallback chain configured. Use the CLI to add models:\n  bun opencode-nim-rotator",
			duration: 5000,
		});
		return;
	}
	...
	const options: SelectOption<string>[] = chain.map((m, i) => {
		let benchStr = "";
		if (
			m.benchmarkStatus === "done" &&
			m.benchmarkTtfb != null &&
			m.benchmarkTps != null
		) {
		...
		return (api.ui as any).DialogSelect({
			...
			onSelect: (opt: SelectOption<string>) => {
				...
				if (model) {
					...
					if (model.benchmarkTtfb != null)
						modelInfo.push(`TTFB: ${model.benchmarkTtfb.toFixed(0)}ms`);
					if (model.benchmarkTps != null)
						modelInfo.push(`TPS: ${model.benchmarkTps.toFixed(1)}`);
					api.ui.dialog.clear();
					api.ui.toast({
						variant: "info",
						title: "NIM Key Rotator",
...
export default { id: "nim-rotator", tui: NimRotatorTuiPlugin };
