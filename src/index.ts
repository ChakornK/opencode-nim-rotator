...
import { loadStore, saveStore, addKey, getNextKey, recordFailure, resetFailures, getActiveKeys } from "./storage.js"
...
function createRotatingFetch(store: KeyStore, config?: KeyStoreConfig) {
  return async function rotatingFetch(input: string | URL | Request, init?: RequestInit): Promise<Response> {
    ...
    const headers = new Headers(init?.headers instanceof Headers ? init.headers : Array.isArray(init?.headers) ? init.headers : init?.headers as Record<string, string> | undefined)
    ...
    try {
      ...
      if (response.status === 401 || response.status === 403) {
        ...
        if (retry && retry.key.id !== next.key.id) {
          ...
          saveStore(store, config)
          const retryResponse = await fetch(input, { ...newInit, headers: retryHeaders })
...
export const NvidiaNimKeyRotator: Plugin = async (input: PluginInput, options?: Record<string, unknown>) => {
  const config: KeyStoreConfig = {
    storePath: options?.storePath as string | undefined,
    rotationStrategy: isValidStrategy(options?.rotationStrategy) ? options!.rotationStrategy : "round-robin",
  }
